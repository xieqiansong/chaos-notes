...
> 性能优化 · 实战案例（真实踩坑与实测量化）。对应 GitHub 工程：[chaos-java/jdk8-platform/jdk8-batch-ingest-demo](https://github.com/xieqiansong/chaos-java/tree/master/jdk8-platform/jdk8-batch-ingest-demo)。

## 一句话

用「内存桶攒批 + 一次 Pipeline」把 Redis 命令数从「=条目数」降到「≈批次数」，并通过「探索-反馈-平滑」在线寻优批量大小——不靠人工定参，自动逼近任意负载/并发下的最优批量。

## 版本与适用

- 框架：Spring Boot 2.7、Spring Data Redis（Lettuce）
- 中间件：Redis 8（`localhost:30102`）
- 关联工程：[chaos-java/jdk8-platform/jdk8-batch-ingest-demo](https://github.com/xieqiansong/chaos-java/tree/master/jdk8-platform/jdk8-batch-ingest-demo)（可 clone 复现：`mvn test` 即一键跑完整基准并自动生成报告，详见工程内 `TEST_REPORT.md`）

## 背景

高吞吐数据汇聚场景（如探针/日志/埋点），每条目一次 Redis 写命令。命令数随条目数**线性放大**，网络往返与 Redis CPU 双双成为瓶颈；想靠「固定批量 + Pipeline」优化，又面临最优批量依赖负载/并发、难以人工定死的困境。

## 现状（逐条直写）与瓶颈

- 实现：`legacy` 每条目一次 `HSET`，命令数 = 条目数。
- 瓶颈：8 汇聚线程压测下吞吐 ≈ 12k/s，`redis/s = 条目数`（1.2 万），命令往返是天花板；批量收益真实存在但**静态批量有最优值**——单线程 flood 下 2048 比 512 高 14%，双线程下 512 反超 2%，人工定死无法兼顾。
...
- **水位触发**：队列积压充足时 `drainTo` 瞬取攒满目标批量 → flush；空转超 `idle-flush-ms` 兜底 flush。
- **批量自适应**（核心）：候选 = 当前批量 × {0.5, 0.8, 1.0, 1.25, 2.0}；以 `explore-prob` 概率试探候选做性能采样；反馈用**指数衰减加权平均**得到各候选吞吐；每 `sample-window` 次 flush 挑「加权最优」并以平滑系数过渡，收敛稳定。
- **固定并发消费线程**：`writer-threads` 个线程直取内存桶并发写（不设第二级任务队列）。
- **核算闭合**：数据只在内存桶一个队列中流转，丢弃只在 `offer` 失败时计入 `dropped`，`written + dropped + errors = 总投递`，可审计。

### 关键实现要点（含踩坑）

1. **尾批卡死丢失**：`drainLoop` 在「队列空 + 本地残批不满」时，`if(!batch.isEmpty())` 每次循环都刷新空闲计时 → `idleFlushNs` 兜底永不触发 → 尾批滞留。修正：仅按 `drainTo` 实际新取条数刷新计时，`while` 退出后刷出残批。
2. **legacy 全量 ClassCastException**：`opsForHash().put(key, System.nanoTime(), item)` 的 hashKey 传 `Long`，而 `StringRedisSerializer` 只接受 String → 每条直接抛异常、legacy 显示 0 吞吐。改为 `String.valueOf(...)`。
3. **static 丢弃数采集缺失**：`BenchEngine` 原只在 `instanceof AdaptiveBatchWriter` 时读 `dropped()`，static 的丢弃恒为 0（假象）。给 `BatchWriter` 接口加 `default dropped()` 统一读取。
4. **dropped 假零黑洞（本轮最大坑）**：曾引入「单承包人 + 写线程池」双层队列，写池任务队列无界 → 承包人把内存桶快速转存进写池、内存桶永不"满" → `dropped` 恒 0；生产(≈8M/s)>>消费(≈23k/s) 的差额全积压在写池队列，`close()` 只消化极小部分，**数千万条随 JVM 退出无声丢失**。**最终方案**：去掉写池与第二级队列，消费线程直接取内存桶——单队列核算天然闭合，`dropped` 恢复真实值。
5. **压测环境隔离**：本地端口转发隧道偶发抖动 + 遗留大 hash 拖慢后续场景 → 每场景前 `DEL key` 隔离；3 分钟级长跑对受限环境压力过大，收敛专项取 60s。
...
- **核算闭合**：flood 丢弃（生产 ≈8M/s >> 消费 ≈24k/s）真实计入 `dropped`，匀速 dropped=0。
...
要点：最优批量不是常数——单线程下 2048 更优、双线程下 512 反而略好。**靠人工定死无法兼顾**，这正是 `adaptive` 在线寻优的价值（自适应两组均 ≈static 最优值）。
...
要点：
- **动态引擎核心实证**：批量大小随实测吞吐反馈持续「探索-反馈」调整，而非定死一个值；低/高候选交替正是探索特性。
- 双线程批量爬升到 3841、吞吐 40176/s、errors=0，命令量较逐条降约 600 倍。
