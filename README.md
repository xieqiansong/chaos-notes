...
| 笔记 | 主题 |
|---|---|
| [多租户分布式限流：本地+Redis 双层限流](performance/多租户分布式限流-本地加Redis双层限流.md) | 限流 / 本地+Redis、精度换性能、压测 |
| [Redis 批量入库：自适应批量大小](performance/Redis批量入库-自适应批量大小.md) | 批量入库 / 攒批、在线寻优、命令量降千倍 |
